...
if(location.href.indexOf('youtown.nl') > -1) {
	CITYLIFE_BASE_URL = "http://youtown.nl";
}
...
function setPos() {
	if (navigator.geolocation && store.get('config.position') == null) {
		...
		navigator.geolocation.getCurrentPosition(successGeo, errorGeo, {timeout:30000});
...
if(location.href.indexOf('app.youtown.nl') < 0) {
...
function formatDateTime(date, time, tz, allday) {
	at = ' at ';
	tz = ' ' + tz; 
	LANGUAGE_CONFIG_SET = store.get('config.language');
...
function getJSONcached(uri, params, cb) {
	...
	//console.log('json-cacher id ', ID, cb, cached);
	...
	if(_.isNull(cached)) {
		...
		//console.log('json cacher cacher run', cached);
		setTimeout(function(){
			cb(cached);
		}, 100);
	}
...
if(store.get('config.language') == null) {
	lang = 'en';
	if(EU) lang = 'nl';
	store.set('config.language', lang);
}
...
$(document).ready(function(){
	...
	if(theUrl.indexOf('__app=true') > -1) {
		$('body').addClass('isApp');
	}
	...
	if(EU) {
		...
		$('#splash').live('pagecreate', function(){
			$('#splash p.www a').html('app.youtown.nl').attr('href', 'http://app.youtown.nl');
		}); 
